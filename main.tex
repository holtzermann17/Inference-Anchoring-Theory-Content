...
%%% Some complaint...
%%% https://tex.stackexchange.com/questions/257829/package-titlesec-error-entered-in-horizontal-mode-sectionintrodu%C3%87%C3%83o
...
%%   {\protect\marginnote[\theparagraph]{\theparagraph}}{0em}{}{\space}

%% \renewcommand{\marginnotevadjust}{-1\baselineskip}

%% \titlespacing{\paragraph}{0.0em}{1ex}{0.5em}
...
%% \thesis{Argumentation theory can help us understand the way mathematical knowledge develops.}
%% In order to better understand the processes of mathematical proof and
%% communication, we turn to informal mathematical dialogues and
%% expository texts as our primary sources of data. Improved
%% understanding of mathematical practice can, in turn, inform the design
%% of support tools for mathematicians.

%% \paragraph{Computational tools for proof checking, automatic theorem proving, and computer algebra are well-established, however, they can only be used with formal, computationally explicit, content.}
%% Translating proofs into formal languages constitutes an unrealistic
%% demand for most mathematicians and students, and does little to address
%% their day-to-day communication needs \cite{thurston1994proof,melis2005proof}.
%% Furthermore, the existing mathematical literature is opaque to current computational systems,
%% which rely on hand-coded knowledge bases entered in formal or
%% constrained languages \cite{furse1990did,furse1994mathematics}.
%% %
%% Both data ingestion and end-user interaction could benefit from the use
%% of an expressive modelling language that is closely related to the way
%% mathematics is actually done.

%% \paragraph{Our approach to modelling mathematical arguments is inspired by the general-purpose argument modelling formalism \emph{Inference Anchoring Theory} (IAT), introduced by \citet{reed2011dialogues}.}
%% As its name suggests, IAT anchors logical inferences in discourse.
%% IAT has been applied to mediation \cite{janier2017towards}, debates
%% \cite{budzynska2014model}, and to paradoxes in ethotic argumentation
%% \cite{budzynska2013circularity}, along with other real-world
%% dialogues \cite{budzynska2013towards}.
%% %%
%% The \emph{Inference Anchoring Theory + Content} (IATC)
%% framework we introduce is based on IAT, but with several
%% significant modifications.  Most fundamentally,
%% IATC is designed to bring to the surface the structural features inherent in mathematical content.

%% \paragraph{Indeed, IATC could be overlayed on top of mathematics represented in more formal languages, where an informal text can be put in full or partial correspondence with a formal treatment.}
%% Lamport's ``Temporal Logic of Actions+'' ({TLA}$^{+}$)
%% \cite{lamport1999specifying,lamport2014tla2} is one candidate
%% formalism that could be used to model content-level expressions, by
%% analogy with Lamport's own semi-formal ``structured proofs''
%% \cite{lamport1995write,lamport2012write21st}.

%% \paragraph{Unlike structured proof, IATC does not aim to reshape the way people do mathematics, but to model it more exactly.}
%% As such, it constitutes groundwork for a future generation of computer systems that can collaborate with mathematicians and students in a way these potential users already understand.
%% \citet{epstein2015wanted} highlights the ``extent to which a person
%% believes that her work experience or product has been facilitated or
%% improved by the collaboration'' as a key evaluation metric for
%% assessing collaborative intelligent computer systems.
%% %%
%% The key metric at this stage is more basic, namely, we are interested
%% in the degree to which IATC can represent real-world examples of
%% mathematical practice in a way that can make them accessible to
%% computational reasoning.  After introducing the modelling approach, we
%% use several examples to show that IATC is indeed satisfactory in this
%% regard.

%% Because these examples form the key piece of empirical evidence, we
%% introduce them briefly here.  This this shows the scope and also the
...
%%   It is argument in the
%% latter sense which is \emph{reason-giving}: it ``expresses a point of
%% view on a question, and offers as support for this position one or
%% more reasons'' \cite[p.~448]{hitchcock2017informal}.
...
%% Reasoning ``enables communicators to produce arguments
%% to convince addressees who would not accept what they say on trust''
%% and, on the other hand, ``enables addressees to evaluate the soundness
%% of these arguments and to accept valuable information that they would
%% be suspicious of otherwise'' (p.~72).
...
%% Proofs are concerned with what necessarily follows from accepted premises
%% according to accepted inference rules.  There are, to be sure, other
%% relevant perspectives on mathematics, for example, the question of how we come to
%% accept or select certain premises and inference rules rather than
%% others.  Here, our concern will be communicated reasoning in
%% mathematical settings, looking at an evidence base that includes but
%% is not limited to deductive proofs.
...
%% Aspects of
%% mathematical communication that go beyond
%% inference, and that support it, are particularly prevalent
%%
...
%% For example, is not appropriate to conclude
%% from $f(n)=f(-n)$ that $n=-n$ \cite[p.~141]{dove2009towards},
%% because functions and numbers are different kinds of mathematical content
%% with different associated rules.
%%
%% %\citet{pedemonte2007} disagrees and sees them as something more general.
%% %%
%% ``Argument'' may in some cases stand for the concept of an
%% , one hallmark being that
%% such presentations admit multiple semantically distinct formalisations
%% .  On the other hand, ``argument'' may stand
%% for a still more abstract 
%% which would not admit formalisation without the addition of
%% significant further details.  Yet again, in mathematics as in other
%% disciplines, the concept of ``argument'' is sometimes associated with
%% social phenomena of 
%%
Other scholars have observed features such as these:
%%
\begin{itemize}
%% The relationship between mathematical reasoning and everyday reasoning
%% has been debated \cite{kisielewicz2015new,aberdein2015commentary}.
...
%% \citet{aberdein2010virtue} 
%% has suggested that for argumentation to be successful,
%%  communication must sufficiently manifest several ``virtues of
%% argument'', broadly typologised as \emph{willingness to engage in
%%   argumentation}, \emph{willingness to listen to others},
%% \emph{willingness to modify one's own position}, and \emph{willingness
...
%% , which is sourced
%% from a public discussion of a problem that appeared in an advanced
%% high school level mathematical competition.


%% \paragraph{Carrascal emphasises that mathematical communication has a rhetorical aspect which depends on context.}
%% %%
%% %%
...
\cite{budzynska2016theoretical}.
%%   The two dialogues 
%% diagrammed in Figures  \ref{fig:TypicalIAT} and \ref{fig:A-not-A}
%% illustrate that statements often cannot be meaningfully understood
%% outside of the conversations in which they are embedded.
Nevertheless, DRT has been successfully applied to model some aspects
of mathematical discourse,
%% (e.g., by \citet{zinn2004understanding}),
...
%%   For example, $A^\prime$ relates explicitly to the content of
%% Bob's original statement ``$A$'', and is modelled as a reformation of that earlier
%% statement.  This suggests a long-range transition between Bob's first and second locutions.  Our current considerations  
...
%% This notation is inspired
%% by McCarthy's m-expressions
%% \citeyearpar{mccarthy1960recursive}, whereas the other more compressed notation is
%% s-expression based and is used when grammatical categories do not need
%% to be explicitly presented, e.g.,
%% `\texttt{(Suggest~(goal~perm\_view\_mod))}' is equivalent to
%% `\texttt{perf[Suggest](meta[goal](perm\_view\_mod))}'.
...
%% The first example concerns an argument, prepared in advance, that informally outlines a solution to a tricky but not particularly advanced mathematical problem.  
\begin{itemize}
...
%% The second example comes from a didactic but nevertheless off-the-cuff discussion of a more technically advanced question.
...
%% The third example again considers a proof, but one that was developed in the context of an experiment in collaborative proof contruction, involving a large number of discussants.
...
%% Section to be reworked without s-expressions 
...
%% In Section \ref{iatc-examples:mathoverflow}, we describe how IATC can
%% make the contents of mathematical dialogues amenable to further computational analysis.
...
%% \thesis{As indicated in the Introduction, a metric for IATC's success
%%   is the degree to which it can represent real-world examples of
%%   mathematical practice in a way that makes them accessible to
%%   computational reasoning.}  This section gives evidence for the
%% plausibility of such an enterprise.

%% \begin{figure}[h]
%% {\centering
%% \fbox{\includegraphics[width=.9\textwidth]{finite-and-infinite-groups}}

%% \par}
%% \caption{Screenshot of a question about group theory posted to MathOverflow:
%% ``I have seen this problem, that if $G$
...
%% \label{fig:mathoverflow-question}}
%% \end{figure}
...
%% \paragraph{As an illustration of our representation strategy, Listing \ref{listing:qa} shows how several \texttt{Assert}ions and one \texttt{Question} written in IATC's s-expression notation can express the proposed analogy between \ref{qa:lhs:prop} and \ref{qa:rhs:prop}, above, in such a way that the statements become available to subsequent computational reasoning.}
%% Further details of the ensuing dialogue could be represented
%% similarly, but this excerpt is sufficient to show the style.
%% The listing is structured to illustrate the interaction between an
%% analyst and an interactive computer system.  
...
%% \begin{listing}{Importing a question that posits and probes an analogy between statements about finite and infinite mathematical groups \label{listing:qa}}
%% {\fontsize{9}{10.5}
%% \begin{Verbatim}[commandchars=\\\{\}]
%% \textbf{1>} (read-tree  '(Assert
%%                  (implies
...
%% \begin{Verbatim}[fontshape=it,commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7}]
%% $\rightarrow$ (Assert123
%%     (implies124
...
%% \vspace{-1.2\baselineskip}
%% \begin{Verbatim}[fontshape=it,commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7}]
%% $\rightarrow$ (Assert128 (analogy129 (implies124 ...)
%%                           (implies130 ...)))
%% \end{Verbatim}
%% \begin{Verbatim}[commandchars=\\\{\}]
%% \textbf{3>} (read-tree '(Question implies130))
%% \end{Verbatim}
%% \vspace{-1.2\baselineskip}
%% \begin{Verbatim}[fontshape=it,commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7}]
%% $\rightarrow$ (Question134 implies130)
%% \end{Verbatim}
%% }
%% \end{listing}
...
\emph{concepts},
%% including analogies,
%% conjecture-generated concepts, (counter)example-generated
%%   concepts and new conventions;
\emph{proofs},
%including proof strategies and the identification of proof-relevant
%  properties;
and \emph{other}.  
%%
...
%% ; one may also retract an assertion as part of gentle apology: ``I
%% misunderstood the question'' in comment 7.2.1.1, and ``No. This
%% example is false. :)'' in comment 14.2.1.1.
...
%%  For example, the least-active window was 9:05-9:09, which featured four comments
%% 9:05 pm perf[define](smallest_angle)	Concept
%% 9:06 pm perf[challenge](forall_exists_problem, too_many)	Other
%% 9:08 pm perf[assert](rel[instance_of](convex_hull_stuck, equi_tri_stuck))\newline perf[query](rel[stronger](unworking_path, convex_hull_stuck))	Conjecture
...
%%
%% developed within the context of the Naproche project---short
%% for ``Natural language Proof Checking''---which aimed to
%% develop ``a controlled natural language (CNL) for mathematical texts
%% which can be transformed automatically into equivalent first order
%% formulas by methods of computational linguistics''
%% \cite{cramer2009naproche}.  %% Despite its long term vision of ``a
%% %% future computer program which could support mathematicians as they
%% %% write their mathematical proofs in the usual language employed by
%% %% mathematicians for this purpose'' \cite[p.~1]{cramer2013proof},
%% However, despite the project's name,
%% in pursuing these aims, the controlled natural language
%% that was developed remained ``far
%% removed from that of the language of mathematics''
%% \cite[p.~15]{ganesalingam2013language}.  
%%
%% Similarly to the Naproche project,
%% Zinn's earlier project had focused on checking proofs.
%% He was able to sucessfully parse and check two elementary
%% proofs written in natural language.
%% A detailed summary of the approach appears in
%% \citet{zinn2006supporting}, and
%% \citet[p.~22]{cramer2013proof} provides
%% a succinct review.
%% The linguistic analysis was deemed shallow
%% by later investigators (\emph{ibid}.).
%% From our point of view, there is another fundamental concern.
%% Zinn took the perspective that
%% \begin{quote}
%% ``the semantics of an informal mathematical argument is its corresponding
%% formal proof, and that discourse understanding in this text genre is defined by generating a formal proof
%% from the informal one'' \cite[p.~1]{zinn2004understanding}.
%% \end{quote}
%% This is philosophically somewhat dubious, as has
%% been argued by \citet{tanswell2015problem},
%% and our examples may also serve to illustrate.
%% Moreover, in such an approach, any parts
%% of mathematical communication not having to do with proof are simply
...
%% This was illustrated in Section \ref{iatc} with an example from 
%% \citet{karttunen1976discourse}.
...
%% As we observed in Section \ref{iatc-examples:mathoverflow}, work on the
%% informal part of mathematics requires attention too.
